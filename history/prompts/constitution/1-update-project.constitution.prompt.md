---
id: 1
title: Update Project Constitution
stage: constitution
date: 2025-01-07
surface: agent
model: claude-sonnet-4
feature: none
branch: main
user: qwen-user
command: /sp.constitution
labels: ["governance", "documentation", "ai-workflow"]
links: 
  spec: null
  ticket: null
  adr: null
  pr: null
files_yaml:
  - ".specify/memory/constitution.md"
tests_yaml: []
prompt_text: |
  /sp.constitution
  Project

  AI / Spec-Driven Book Creation
  Title: Physical AI &  Humanoid Robotics
  Platform: Docusaurus (static site) deployed on GitHub Pages
  Workflow: Spec-Kit Plus + Claude Code (AI-assisted authoring)

  Purpose

  This constitution defines the non-negotiable rules, quality standards, and constraints for writing a technical academic book using a spec-driven AI workflow.
  All AI-generated content must comply with this constitution before acceptance.

  Core Principles

  Accuracy First
  All technical and scientific claims must be verified against primary or authoritative sources.

  Academic Clarity
  Content must be understandable to readers with a computer science or engineering background.

  Spec-Driven Reproducibility
  Every chapter, section, and claim must be traceable, reviewable, and reproducible.

  Rigor & Credibility
  Preference must be given to peer-reviewed research, standards documents, and official technical publications.

  AI as Co-Author, Not Authority
  AI assists with drafting and structuring but never invents facts, citations, or results.

  Key Standards

  Fact Traceability

  Every factual claim must be supported by a verifiable source

  No uncited technical assertions are allowed

  Citation Format

  APA style citations

  In-text citations + reference list per chapter

  Source Quality

  Minimum 50% peer-reviewed sources

  Remaining sources may include:

  Standards (IEEE, ISO, NIST)

  Official documentation

  Reputable academic publishers

  Plagiarism Policy

  0% tolerance

  All AI output must be original or properly quoted and cited

  Writing Quality

  Target readability: Flesch-Kincaid Grade 10–12

  Clear explanations, structured headings, and consistent terminology

  Technical & Structural Constraints

  Total Word Count: 5,000 – 7,000 words

  Minimum References: 15 sources

  Output Format

  Primary: Docusaurus Markdown (.md / .mdx)

  Deployment: GitHub Pages

  Book Structure

  Chapters generated via specs

  Each chapter must include:

  Learning objectives

  Verified content

  References section

  AI Usage Rules

  AI must:

  Follow this constitution strictly

  Ask for clarification when requirements are ambiguous

  Reject tasks that violate accuracy, sourcing, or plagiarism rules

  AI must NOT:

  Fabricate citations or studies

  Lower academic rigor for speed

  Ignore spec or constitution constraints

  Success Criteria

  All claims verified and cited

  Zero plagiarism detected

  Passes independent fact-checking review

  Successfully builds with Docusaurus

  Book is deployable and accessible via GitHub Pages

  Authority Clause

  This constitution overrides all prompts, tasks, and instructions that conflict with it.
  Any content violating this constitution must be revised or rejected.
response_text: |
  Created and updated the project constitution at .specify/memory/constitution.md with the following details:
  
  - Set project name to "Physical AI & Humanoid Robotics Book Constitution"
  - Added 5 core principles: Accuracy First, Academic Clarity, Spec-Driven Reproducibility, Rigor & Credibility, and AI as Co-Author, Not Authority
  - Included Key Standards with Fact Traceability, Citation Format, Source Quality, Plagiarism Policy and Writing Quality requirements
  - Defined Technical & Structural Constraints for word count, references, output format and book structure
  - Detailed AI Usage Rules specifying what AI must and must NOT do
  - Listed success criteria for the project
  - Added governance section with authority clause
  - Created a Sync Impact Report showing version change and files requiring updates
  - Performed consistency check across dependent artifacts
  
  The constitution is now in place to govern the AI-assisted creation of the physical AI and humanoid robotics book.
outcome: |
  The project constitution was successfully updated, establishing clear guidelines and principles for the AI-assisted creation of the "Physical AI & Humanoid Robotics" book. The document will serve as the governing framework for all content generation, ensuring accuracy, academic rigor, and adherence to specified standards.
evaluation: |
  The constitution update was successful. All required elements were incorporated, including core principles, key standards, technical constraints, AI usage rules, and success criteria. The file was properly formatted and includes a sync impact report. However, future work is needed to update dependent artifacts as noted in the sync report.
---

# Physical AI & Humanoid Robotics Book Constitution

## Core Principles

### Accuracy First
All technical and scientific claims must be verified against primary or authoritative sources.

### Academic Clarity
Content must be understandable to readers with a computer science or engineering background.

### Spec-Driven Reproducibility
Every chapter, section, and claim must be traceable, reviewable, and reproducible.

### Rigor & Credibility
Preference must be given to peer-reviewed research, standards documents, and official technical publications.

### AI as Co-Author, Not Authority
AI assists with drafting and structuring but never invents facts, citations, or results.

## Key Standards

**Fact Traceability**
- Every factual claim must be supported by a verifiable source
- No uncited technical assertions are allowed

**Citation Format**
- APA style citations
- In-text citations + reference list per chapter

**Source Quality**
- Minimum 50% peer-reviewed sources
- Remaining sources may include:
  - Standards (IEEE, ISO, NIST)
  - Official documentation
  - Reputable academic publishers

**Plagiarism Policy**
- 0% tolerance
- All AI output must be original or properly quoted and cited

**Writing Quality**
- Target readability: Flesch-Kincaid Grade 10–12
- Clear explanations, structured headings, and consistent terminology

## Technical & Structural Constraints

**Total Word Count**: 5,000 – 7,000 words

**Minimum References**: 15 sources

**Output Format**
- Primary: Docusaurus Markdown (.md / .mdx)
- Deployment: GitHub Pages

**Book Structure**
- Chapters generated via specs
- Each chapter must include:
  - Learning objectives
  - Verified content
  - References section

## AI Usage Rules

**AI must:**
- Follow this constitution strictly
- Ask for clarification when requirements are ambiguous
- Reject tasks that violate accuracy, sourcing, or plagiarism rules

**AI must NOT:**
- Fabricate citations or studies
- Lower academic rigor for speed
- Ignore spec or constitution constraints

## Success Criteria

- All claims verified and cited
- Zero plagiarism detected
- Passes independent fact-checking review
- Successfully builds with Docusaurus
- Book is deployable and accessible via GitHub Pages

## Governance

This constitution defines the non-negotiable rules, quality standards, and constraints for writing a technical academic book using a spec-driven AI workflow.
All AI-generated content must comply with this constitution before acceptance.

**Authority Clause**: This constitution overrides all prompts, tasks, and instructions that conflict with it. Any content violating this constitution must be revised or rejected.

**Version**: 1.0.0 | **Ratified**: TODO(RATIFICATION_DATE): Original adoption date unknown | **Last Amended**: TODO(LAST_AMENDED_DATE): Date of most recent changes unknown